...
def create_mesh(geom: GeometricParameters):
    ...
    # # ジオメトリの点と線を定義
    # p_axis_bottom = gmsh.model.geo.addPoint(0, -sic_depth_dimless - sio2_depth_dimless, 0)
    # p_axis_sic_sio2 = gmsh.model.geo.addPoint(0, -sio2_depth_dimless, 0)
    # p_axis_sio2_vac = gmsh.model.geo.addPoint(0, 0, 0)
    
    # tip_center = gmsh.model.geo.addPoint(0, tip_z_dimless + tip_radius_dimless, 0)
    # tip_apex = gmsh.model.geo.addPoint(0, tip_z_dimless, 0)
    # tip_tangent = gmsh.model.geo.addPoint(
    #     tip_radius_dimless * np.cos(tip_slope_angle),
    #     tip_z_dimless + tip_radius_dimless * (1 - np.sin(tip_slope_angle)),
    #     0,
    # )
    # r_tangent = tip_radius_dimless * np.cos(tip_slope_angle)
    # z_tangent = tip_z_dimless + tip_radius_dimless * (1 - np.sin(tip_slope_angle))
    # tip_top = gmsh.model.geo.addPoint(r_tangent + (vac_depth_dimless - z_tangent) * np.tan(tip_slope_angle), vac_depth_dimless, 0)

    # p_far_bottom = gmsh.model.geo.addPoint(R_dimless, -sic_depth_dimless - sio2_depth_dimless, 0)
    # p_far_sic_sio2 = gmsh.model.geo.addPoint(R_dimless, -sio2_depth_dimless, 0)
    # p_far_sio2_vac = gmsh.model.geo.addPoint(R_dimless, 0, 0)
    # p_far_top = gmsh.model.geo.addPoint(R_dimless, vac_depth_dimless, 0)
    
    # # ジオメトリの線と面を生成
    # l_axis_sic = gmsh.model.geo.addLine(p_axis_bottom, p_axis_sic_sio2)
    # l_interface_sic_sio2_axis = gmsh.model.geo.addLine(p_axis_sic_sio2, p_far_sic_sio2)
    # l_far_sic = gmsh.model.geo.addLine(p_far_sic_sio2, p_far_bottom)
    # l_ground = gmsh.model.geo.addLine(p_far_bottom, p_axis_bottom)
    
    # l_interface_sio2_vac_axis = gmsh.model.geo.addLine(p_far_sio2_vac, p_axis_sio2_vac)
    # l_axis_sio2 = gmsh.model.geo.addLine(p_axis_sio2_vac, p_axis_sic_sio2)
    # l_far_sio2 = gmsh.model.geo.addLine(p_far_sio2_vac, p_far_sic_sio2)
    
    # l_axis_vac = gmsh.model.geo.addLine(p_axis_sio2_vac, tip_apex)
    # l_tip_arc = gmsh.model.geo.addCircleArc(tip_apex, tip_center, tip_tangent)
    # l_tip_shaft = gmsh.model.geo.addLine(tip_tangent, tip_top)
    # l_top_boundary = gmsh.model.geo.addLine(tip_top, p_far_top)
    # l_far_vac = gmsh.model.geo.addLine(p_far_top, p_far_sio2_vac)

    # # 領域 (Surface) を定義
    # surf_sic = gmsh.model.geo.addPlaneSurface([gmsh.model.geo.addCurveLoop([l_axis_sic, l_interface_sic_sio2_axis, l_far_sic, l_ground])])
    # surf_sio2 = gmsh.model.geo.addPlaneSurface([gmsh.model.geo.addCurveLoop([l_axis_sio2, l_interface_sic_sio2_axis, l_far_sio2, l_interface_sio2_vac_axis])])
    # surf_vac = gmsh.model.geo.addPlaneSurface([gmsh.model.geo.addCurveLoop([l_axis_vac, l_tip_arc, l_tip_shaft, l_top_boundary, l_far_vac, l_interface_sio2_vac_axis])])
    # gmsh.model.geo.synchronize()

    # # メッシュサイズ制御
    # # 探針先端と界面近傍でメッシュを細かくする
    # gmsh.model.mesh.field.add("Distance", 1, points=[tip_apex])
    # gmsh.model.mesh.field.add("Threshold", 2, InField=1, SizeMin=1, SizeMax=10, DistMin=1, DistMax=R_dimless / 5)
    # gmsh.model.mesh.field.add("Distance", 3, curves=[l_interface_sic_sio2_axis, l_interface_sio2_vac_axis, l_axis_sio2])
    # gmsh.model.mesh.field.add("Threshold", 4, InField=3, SizeMin=2.5, SizeMax=10, DistMin=10, DistMax=sic_depth_dimless / 2)
    # gmsh.model.mesh.field.add("Min", 5, FieldsList=[2, 4])
    ...
    # gmsh.model.addPhysicalGroup(1, [l_ground], 11, "ground")
    # gmsh.model.addPhysicalGroup(1, [l_axis_sic, l_axis_sio2, l_axis_vac], 12, "axis")
    # gmsh.model.addPhysicalGroup(1, [l_far_sic, l_far_sio2, l_far_vac, l_top_boundary], 13, "far_field")
    # gmsh.model.addPhysicalGroup(1, [l_tip_arc, l_tip_shaft], 14, "tip")
    # gmsh.model.addPhysicalGroup(1, [l_interface_sio2_vac_axis], 15, "sic_sio2_interface")
