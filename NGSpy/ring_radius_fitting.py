...
def main(sim_path, exp_vtip_path, exp_htip_path, *, fixed_Vtip, fixed_Htip, rmax, Htip_max, Vtip_max, exp_zoffset):
    ...
    potentials = potentials[:, :, :, r <= rmax]
    r = r[r <= rmax]
    
    potentials = potentials[:, :, Htip_values <= Htip_max, :]
    Htip_values = Htip_values[Htip_values <= Htip_max]

    potentials = potentials[Vtip_values <= Vtip_max, :, :, :]
    Vtip_values = Vtip_values[Vtip_values <= Vtip_max]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=150.0,
        help="Maximum ring radius (nm) to consider",
    )
    parser.add_argument(
        "--htip_max",
        type=float,
        default=50.0,
        help="Maximum Htip (nm) to consider",
    )
    parser.add_argument(
        "--vtip_max",
        type=float,
        default=5.0,
        help="Maximum Vtip (V) to consider",
    )
    parser.add_argument(
        "--exp_zoffset",
        type=float,
    ...
    main(
        ...
        rmax=args.rmax,
        Htip_max=args.htip_max,
        Vtip_max=args.vtip_max,
