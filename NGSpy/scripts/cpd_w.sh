...
OUTPATH=outputs/${DATANAME}_cpd_w
mkdir -p outputs
mkdir -p $OUTPATH
...
cat $ARGSFILE | xargs -n 3 -P $NPROCS sh -c '
    ...
    python main_fast.py --V_tip="-4:-0.2:0.2" --tip_radius "$1" --tip_height "$2" --out_dir "$3" --sigma_s 1E11 >> "$3/main.job.log" 2>&1 --point_charge  && \
    python main_fast.py --V_tip="0:4:0.2" --tip_radius "$1" --tip_height "$2" --out_dir "$3" --sigma_s 1E11 >> "$3/main.job.log" 2>&1 --point_charge && \
